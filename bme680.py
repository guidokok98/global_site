...
class bme680:
    ...
    def readReg(self,reg):
        regVal = self.i2cBus.read_byte_data(self.addr, reg)
        return regVal

    #write data to one register
    def writeReg(self,reg,val):
        self.i2cBus.write_byte_data(self.addr, reg, val)
    ...
    def getTemp(self):
        ...
        MSB = self.readReg(self.tempRegs[0])
        LSB = self.readReg(self.tempRegs[1])
        XLSB = self.readReg(self.tempRegs[2])
    ...
    def getPress(self):
        ...
        MSB = self.readReg(self.pressRegs[0])
        LSB = self.readReg(self.pressRegs[1])
        XLSB = self.readReg(self.pressRegs[2])
    ...
    def getHumi(self):
        ...
        MSB = self.readReg(self.humiRegs[0])
        LSB = self.readReg(self.humiRegs[1])
    ...
    def getGasRes(self):
        if ((self.readReg(0x2B)&0b100000) == 0):
            return None
        MSB = self.readReg(self.gasRegs[0])
        LSB = self.readReg(self.gasRegs[1])
        ...
        var1 = (((1340+(5*range_switching_error))*(self.const_array1_int[gas_range]))>>16)
        var2 = (self.adcGas << 15) - (1 <<24) +var1
        self.gasRes = ((((self.const_array2_int[gas_range]*var1)>>9)+(var2>>1))/var2)
        return self.gasRes
